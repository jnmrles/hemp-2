...
const Service = () => {
  return (
    <section className="  pt-0 pb-0   overflow-hidden bg-cover bg-fixed  bg-no-repeat bg-[60%] bg-[url('../assets/cbd2.jpg')]">
  ...
  return (
    <>
      <div className="w-full  px-4 md:w-1/2 lg:w-1/3">
        <motion.div 
       className="mb-8  w-full bg-gray-400 rounded-md bg-clip-padding backdrop-filter backdrop-blur-sm bg-opacity-30 border border-slate-400 rounded-[20px] bg-white p-10 shadow-md hover:shadow-2xl hover:shadow-white md:px-7 xl:px-10">
          <div
            className={`mb-8 flex h-[70px] w-[70px] p-3 items-center justify-center rounded-2xl bg-black/50`}
          >
            {icon}
          </div>
          <h4 className="mb-3 text-xl font-semibold text-dark">{title}</h4>
          <p className="text-grey-200">{details}</p>
        </motion.div>
      </div>
    </>
  );
          ...
          {({products}) => (
            <div className="recommended-products-grid p-2" >
              {products.nodes.map((product) => (
                <motion.div className=' p-4  bg-gray-400 rounded-md bg-clip-padding backdrop-filter backdrop-blur-sm bg-opacity-30 border border-gray-100' whileHover={{ scale: 1.1, duration:.1 }}
                whileTap={{ scale: 0.9 }}
                initial="hidden"
                whileInView="visible"
